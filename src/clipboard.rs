...
extern "C" {
    ...
    // Write data of type to the clipboard. This method will clear the clipboard and replace with the given data. Hence another call to this method will overwrite with the new one. To write multiple types of data to clipboard simultaneously, you will need to use clip.set(clipboardDataList) below.
    ...
    // clip.set(clipboardData)
    // clipboardData {Object} JSON object containing data, type and raw to be written to clipboard. See clip.set(data, [type, [raw]]) for detailed arguments specification.
    // clip.set(clipboardDataList)
    // clipboardDataList {Array} Array of clipboardData to be written to clipboard. See clip.set(clipboardData) and clip.set(data, [type, [raw]]) for detailed arguments specification.
    ...
    // clip.get(clipboardData)
    // clipboardData {Object} JSON object containing type and raw argument for reading data from clipboard. See clip.get([type, [raw]]) for detailed arguments specification.
    // Returns {String} the data retrieved from the clipboard
    // clip.get(clipboardDataList)
    // clipboardDataList {Array} Array of clipboardData for reading data from clipboard. Multiple types of data can be read from clipboard simultaneously with this method. See clip.get(clipboardData) and clip.get([type, [raw]]) for detailed arguments specification.
